...
CInputManager::CInputManager()
    : m_input(ftxui::Input(&m_szInput, "Type here...", {
        .transform = [](ftxui::InputState state) {
...
void CInputManager::inputLoop() {
    ...
    while (true) {
        ...
        g_pCommandHandler->isCommand(input) ? 
            g_pCommandHandler->handleCommand(input) : 
            g_pChatManager->newMessage({.msg = input, .username = "Server"});
    }
}
